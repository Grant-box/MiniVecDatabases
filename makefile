...
# 编译选项
CXXFLAGS = -std=c++11 -g -Wall  # 设置编译选项：使用 C++11 标准，开启调试信息，显示所有警告
...
# 构建目录
# BUILD_DIR = build  # 定义存放对象文件的目录

# 对象文件
OBJECTS = $(patsubst %.cpp, build/%.o, $(SOURCES))  # 将源文件名转换为对象文件名（.o）
...
# 编译源文件为对象文件
%.o: %.cpp  # 规则：将 .cpp 文件编译为 .o 文件
	$(CXX) $(CXXFLAGS) -c $< -o build/$@  # 使用编译器编译源文件，生成对象文件

# 清理目标
clean:  # 定义清理目标
	rm -f build/%.o $(TARGET)  # 删除所有对象文件和可执行文件
